...
def trial(config_update):
    ...
    cache_key = (config.data_name, experiment_name)
    if cache_key in _TRIAL_CACHE:
        print("cache_hit=True (reusing data + model + precomputed outputs)")
        ...
        valid_x_tensor = cached["valid_x_tensor"]
        valid_y_tensor = cached["valid_y_tensor"]
        test_x_tensor = cached["test_x_tensor"]
        test_y_tensor = cached["test_y_tensor"]
        valid_logit = cached["valid_logit"]
        valid_u = cached["valid_u"]
        test_logit = cached["test_logit"]
        test_u = cached["test_u"]
        test_sigma2_cached = cached["test_sigma2_cached"]
        model = cached["model"]
        print(f"split_sizes valid={len(valid_y_tensor)} test={len(test_y_tensor)}")
        print(f"backbone_loaded model_class={model.__class__.__name__} (cached)")
    else:
        print("cache_hit=False (loading data + model + precomputing)")
        ...
        _TRIAL_CACHE[cache_key] = {
            ...
            "test_u": test_u,
            "test_sigma2_cached": test_sigma2_cached,
            "model": model,
        }
        del data, train, valid, test, valid_x, valid_y, test_y
        del valid_tensor_data, test_tensor_data
    ...
    test_x = [test_x_tensor[:, i:i+1].numpy() for i in range(test_x_tensor.shape[1])]
