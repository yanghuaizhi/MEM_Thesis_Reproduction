...
def trial(config_update):
    ...
    if cache_key in _STA_CACHE:
        print("cache_hit=True (reusing data + model)")
        cached = _STA_CACHE[cache_key]
        ...
        valid_tensor_data = cached["valid_tensor_data"]
        test_tensor_data = cached["test_tensor_data"]
        print(f"split_sizes valid={len(valid_tensor_data)} test={len(test_tensor_data)}")
        print(f"backbone_loaded model_class={model.__class__.__name__} (cached)")
    else:
        print("cache_hit=False (loading data + model)")
